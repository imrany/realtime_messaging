...
export default function projects() {
    ...
    let [projectYear,setProjectYear]=useState("");
    ...
    function handleProjectYearValue(value:string){
        if(value){
        ...
        <div className={md_screen===false?"min-h-[60vh] md:p-10 max-md:py-10 justify-center flex gap-4 max-sm:flex-wrap":"min-h-[60vh] md:p-10 max-md:py-10 flex gap-4 max-sm:flex-wrap"}>
            {!showProjects?(
                <div className="flex flex-col max-md:mt-8">
                    <div className="md:w-[40vw] h-fit md:shadow-sm rounded-sm md:shadow-slate-400 px-2 pb-6">
                        <p className="font-semibold text-2xl ml-2 my-3">Projects</p>
                        <Select
                            className=" max-md:w-[90vw] placeholder:text-slate-700"
                            placeholder="Choose project year :"
                            options={project_year_options}
                            onChange={(e:any)=>handleProjectYearValue(e.value)}
                        />
                    </div>
                </div>
            ):(
                <>
                {showTable?(
                    <div className="mt-8 flex-grow rounded-lg md:border-[1px] text-sm">
                        <div className="flex flex-col py-6 px-8 max-md:px-4 border-b-[1px]">
                            <div className="flex justify-between items-center">
                                <p className="text-[20px] text-[var(--gray-heading)] font-semibold">{projectYear} Projects</p>
                                <button 
                                    onClick={()=>{
